...
test('User lifecycle: create -> login -> get -> update -> logout -> delete -> verify gone', async ({ api }) => {
  ...
  await test.step('create', async () => {
    const created = await api.path('/user').body(user).postRequest(200);
  ...
  await test.step('get', async () => {
    const fetched = await api.path(`/user/${user.username}`).getRequest(200);
  ...
  await test.step('update', async () => {
    await api.path(`/user/${user.username}`).body({ ...user, email: newEmail }).putRequest(200);
    const refetched = await api.path(`/user/${user.username}`).getRequest(200);
  ...
  await test.step('delete', async () => {
    await api.path(`/user/${user.username}`).deleteRequest(200);
  });

  await test.step('verify gone', async () => {
    await api.path(`/user/${user.username}`).getRequest(404);
  });
});
