...
test('PUT /user/{username} updates an existing user', async ({ api }) => {
  ...
  await api.path('/user').body(original).postRequest(200);
  ...
  const putResponse = await api.path(`/user/${original.username}`).body(updated).putRequest(200);
  expect(putResponse).shouldMatchSchema('ApiResponse');

  const fetched = await api.path(`/user/${original.username}`).getRequest(200);
...
test('PUT /user/{username} on a non-existent username behaves per documented contract', async ({ api }) => {
  ...
  const { status } = await api.path(`/user/${ghost}`).body(payload).sendRaw('PUT');
