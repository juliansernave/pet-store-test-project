...
test('POST /user/createWithArray creates multiple users and each is retrievable', async ({ api }) => {
  ...
  const response = await api.path(routes.user.createWithArray).body(payload).postRequest(200);
  expect(response).shouldMatchSchema('ApiResponse');

  for (const user of payload) {
    const fetched = (await api.path(routes.user.byUsername(user.username)).getRequest(200)) as User;
...
test('POST /user/createWithArray with a non-array body is rejected', async ({ api }) => {
  ...
  const { status } = await api.path(routes.user.createWithArray).body(singleUser).sendRaw('POST');
  ...
  if (status === 200) {
    const { status: getStatus } = await api.path(routes.user.byUsername(singleUser.username)).sendRaw('GET');
...
test('POST /user/createWithList creates multiple users and each is retrievable', async ({ api }) => {
  ...
  const response = await api.path(routes.user.createWithList).body(payload).postRequest(200);
  expect(response).shouldMatchSchema('ApiResponse');

  for (const user of payload) {
    await api.path(routes.user.byUsername(user.username)).getRequest(200);
  }
});
